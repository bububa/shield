...
func NewChineseTokenizer(dict string, searchMode bool) Tokenizer {
    var segmenter sego.Segmenter
    if len(dict) == 0 {
        dict = "/var/code/go/src/github.com/huichen/sego/data/dictionary.txt"
    }
    segmenter.LoadDictionary(dict)
    return &zhTokenizer{segmenter:segmenter, searchMode:searchMode}
}

func (t *zhTokenizer) Tokenize(text string) (words map[string]int64) {
    segments := t.segmenter.Segment([]byte(text))
    splittedWords := sego.SegmentsToSlice(segments, t.searchMode)
    words = make(map[string]int64)
    for _, w := range splittedWords {
        if tokenRx.MatchString(w) || len(w) < 2 {
            continue
        }
